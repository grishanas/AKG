...
Model* model1 = NULL;
Camera* cam = NULL;
sf::Vector3f lightPos = { 0.f, 0.f, -500.f };
//Model *model2 = NULL;
...
void PhongShading(sf::Vector3f screenCoords[3], sf::Vector3f vectorsNormals[3], Uint32 color , sf::Vector3f lightPosition)
{
    ...
    float backgroundLiteKoef = 0.1f;
    sf::Vector3f BackgroundColor = {0.f,9.f,0.f} ;
    sf::Vector3f backgroundLite = backgroundLiteKoef * BackgroundColor;
    for (int i = 0; i < totalHeight; i++)
    {
        ...
        for (int j = A.x; j <= B.x; j++) 
        {
            ...
            if (angle < 0)
                angle = 0;
            sf::Vector3f V = normalPixel-cam->getDecartPos();
            sf::Vector3f  diffuseLightFactor = {0.4f,0.33f,0.25f};
            sf::Vector3f diffuseLightColor = { 255,255,255 };
            sf::Vector3f diffuseLight = { diffuseLightColor.x * diffuseLightFactor.x * angle,diffuseLightColor.y * diffuseLightFactor.y * angle, diffuseLightColor.z * diffuseLightFactor.z * angle };

            sf::Vector3f illuminasitionFactor = { 0.2f,0.6f,0.32f};
            sf::Vector3f illuminationColor = { 0.5,0.53,0.2 };
            sf::Vector3f R = Normilize(lightVector - ((DotProduct(lightVector, normalPixel) * normalPixel) * 2.f));
            sf::Vector3f illuminationLight = { illuminasitionFactor.x*pow(DotProduct(R,V),1.f) * illuminationColor.x,illuminasitionFactor.y * pow(DotProduct(R,V),1.f) * illuminationColor.y,illuminasitionFactor.z * pow(DotProduct(R,V),1.f) * illuminationColor.z};

            sf::Vector3f Color = illuminationLight + diffuseLight + backgroundLite;

            if (PixDepth[(int)P.y][(int)P.x] <= P.z)
            {
                PixDepth[(int)P.y][(int)P.x] = P.z;
                pixarray[(int)P.y][(int)P.x].r = Color.x>0xff?0xff:Color.x;
                pixarray[(int)P.y][(int)P.x].g = Color.y>0xff?0xff:Color.y;
                pixarray[(int)P.y][(int)P.x].b = Color.z>0xff?0xff:Color.z;
